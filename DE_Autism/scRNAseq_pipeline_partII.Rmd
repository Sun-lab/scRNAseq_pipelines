...
read_depth = readRDS(paste0(data_dir,"rawM_read_depth_per_1Kcell_ind.rds"))
read_depth_median=median(read_depth)
individual_index=match(t_meta$individual,rownames(read_depth))
...
```{r count_reconstruction_fitting,eval=runcode, fig.dim = c(12,16)}
...
op=par(mfrow = c(3, 4), pty = "s", bty="n")
  ...
  for(i_ind in 1:length(cur_individual)){
    ...
    if(i_g<=4 & i_ind<=3){
      ...
      ggplot(cur_sim_ind, aes(x=simulated_count),stat="count") + geom_histogram(fill="lightblue")+
        labs(title=paste0("Histogram of dca, ",rownames(sub_mean)[i_g]," of ",cur_ind,", ",cur_cluster),x="Simulated Count", y = "Frequency")
      #+theme_classic()
    }
  }
  if(i_g%%500==0){
