...
#(1) the genes with perm_pval1_rate are usually have less expression cases(i.e.<10).
...
plot(-log10(ksgreater),-log10(ksless),cex=.2,
     main=paste0("sig_KSgreater: ",round(sum(ksgreater<0.05)*100/length(ksgreater),3),
                 "%, sig_KSless: ",round(sum(ksless<0.05)*100/length(ksless),3),"%"))
ks_sig_flag=pmin(ksless,ksgreater)<0.05
...
plot(-log10(ksgreater0.2),-log10(ksless0.2),cex=.2,
     main=paste0("sig_KSgreater0.2: ",round(sum(ksgreater0.2<0.05)*100/length(ksgreater0.2),3),
                 "%, sig_KSless0.2: ",round(sum(ksless0.2<0.05)*100/length(ksless0.2),3),"%"))
...
for(i in sample.int(3000,100,replace=FALSE)){
  if(perm_pval1_rate[i]<0.2 && ksless[i]<0.05){
    non_zero_flag=exprM[i,]>0
    hist(exprM[i,non_zero_flag],breaks=20,main=paste0("log expression of gene#",i,
                                                      ", pval ob=", round(fit_ob_pval[i],4),
                                                      ", non-zero num=",exprM_0zero_num[i]),
...
hist(fit_ob_pval[perm_pval1_rate<0.2 & ksless<0.05],breaks=20, main="observed pvalues with pval1_rate<0.2,ksless sig")
hist(fit_ob_pval[perm_pval1_rate<0.2 & ksgreater<0.05],breaks=20, main="observed pvalues with pval1_rate<0.2,ksgreater sig")
hist(fit_ob_pval[perm_pval1_rate<0.2 & ksgreater>=0.05 & ksless>0.05],breaks=20, main="observed pvalues with pval1_rate<0.2,ks no sig")

hist(rawp[perm_pval1_rate<0.2 & ksless<0.05],breaks=20, main="permutation pvalues with pval1_rate<0.2,ksless sig")
hist(rawp[perm_pval1_rate<0.2 & ksgreater<0.05],breaks=20, main="permutation pvalues with pval1_rate<0.2,ksgreater sig")
hist(rawp[perm_pval1_rate<0.2 & ksgreater>=0.05 & ksless>0.05],breaks=20, main="permutation pvalues with pval1_rate<0.2,ks no sig")

hist(fit_perm[perm_pval1_rate<0.2 & ksless<0.05,],breaks=20, main="pvalues from permutation data with pval1_rate<0.2,ksless sig")
hist(fit_perm[perm_pval1_rate<0.2 & ksgreater<0.05,],breaks=20, main="pvalues from permutation data with pval1_rate<0.2,ksgreater sig")
hist(fit_perm[perm_pval1_rate<0.2 & ksgreater>=0.05 & ksless>0.05,],breaks=20, main="pvalues from permutation data with pval1_rate<0.2,ks no sig")
