...
```{r sim_check,eval=runcode, fig.dim = c(9,15)}
...
```{r meta_data,eval=runcode,fig.dim = c(5,5)}
...
ind_covariate_flag=NA  #c(NA, "ind") the individual level covariate for regression, only works for permanova-S method
...
b1 = zlm(formula = ~ diagnosis + ( 1 | ind ), sca = sca, method = 'glmer', ebayes = FALSE, parallel = TRUE)
...
```{r perm_save_results,eval=runcode, fig.dim = c(6,6)}
df2=NA
