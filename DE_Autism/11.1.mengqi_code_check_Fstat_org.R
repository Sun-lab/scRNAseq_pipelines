...
###########input###############
#meta input
tmeta = read.table("/fh/fast/sun_w/mengqi/1.Testing_scRNAseq/github/ideas/Autism/R/res/step4_meta_org.tsv",header = TRUE, sep = "\t")
dim(tmeta)
tmeta[1:2,]
...
#dca input
sub_mean = read.table("/fh/scratch/delete90/sun_w/autism/dca_PFC_all/L2_3_mean.tsv.gz",header = TRUE, sep = "\t")
sub_dispersion = read.table("/fh/scratch/delete90/sun_w/autism/dca_PFC_all/L2_3_dispersion.tsv.gz",header = TRUE, sep = "\t")
sub_dropout   = read.table("/fh/scratch/delete90/sun_w/autism/dca_PFC_all/L2_3_pi.tsv.gz",header = TRUE, sep = "\t")
...
sub_mean[1:2,1:5]
sub_dispersion[1:2,1:5]
sub_dropout[1:2,1:5]

sub_mean = data.matrix(sub_mean[,-1])
sub_dispersion = data.matrix(sub_dispersion[,-1])
sub_dropout   = data.matrix(sub_dropout[,-1])

colnames(sub_mean)=meta$cell
colnames(sub_dispersion)=meta$cell
colnames(sub_dropout)=meta$cell

# test
# sub_mean=sub_mean[1:10,]
# sub_dispersion=sub_dispersion[1:10,]
# sub_dropout=sub_dropout[1:10,]
...
dim(klmean_direct_array)
dim(jsd_direct_array)
dim(klmean_nbzinb_array)
dim(jsd_nbzinb_array)
dim(klmean_empirical_array)
dim(jsd_empirical_array)

klmean_direct_array[1:2,1:2,1:2]
jsd_direct_array[1:2,1:2,1:2]
klmean_nbzinb_array[1:2,1:2,1:2]
jsd_nbzinb_array[1:2,1:2,1:2]
klmean_empirical_array[1:2,1:2,1:2]
jsd_empirical_array[1:2,1:2,1:2]
        ...
        if(file.exists(paste0("/fh/fast/sun_w/mengqi/1.Testing_scRNAseq/Command/11.1.mengqi_code_check/",dist_method,"_",fit_method,"_array_",resid_flag,covariate_flag,fit_tag,pre_tag,"_sim_",cluster_tag,"_",file_tag,".rds"))){
          ...
          for(perm_label in perm_label_seq){
            ...
            if(ind_covariate_flag=="ind"){
              #
              cur_covariate=meta[match(cur_individual,meta$individual),c("age","sex","RIN","Seqbatch")]
              #read depth
              if(length(grep("rdpadj",file_tag))==0){ #if not justed before
                read_depth=readRDS(paste0("../",dataset_folder,"/rawMPFC3k_read_depth_per_1Kcell_ind.rds"))
                read_depth=read_depth[match(cur_individual,sapply(rownames(read_depth),function(x){return(unlist(strsplit(x,"_"))[1])}))]
