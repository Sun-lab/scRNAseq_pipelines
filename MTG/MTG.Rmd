...
date()
e_out
is.cell = (e_out$FDR <= 0.01)

# Temp save image
post_eDrops_fn = file.path(data_dir,"post_eDrops.rds")
if(where_file == "longleaf"){
  saveRDS(list(sce=sce,e_out=e_out,is.cell=is.cell),post_eDrops_fn)
} else if(FALSE){
...
plot(e_out$Total, -e_out$LogProb, col=ifelse(is.cell, "red", "black"),
...
table(is.cell, e_out$Total >= bcrank$inflection)
w2kp = which(is.cell & e_out$Total >= bcrank$inflection)
sce = sce[,w2kp]
dim(sce)
```
