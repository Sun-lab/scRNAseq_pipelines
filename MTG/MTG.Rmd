...
Change the following `src_dir` and `data_dir` variables to your own respective local working directory.
```{r}
...
sce = calculateQCMetrics(sce, feature_controls=list(ERCC=isSpike(sce),Mt=is_mito, Ri=is_ribo))
...
hist(sce$pct_counts_ERCC,xlab="ERCC proportion(%)",
        ylab="Number of cells",breaks=30,main="",col="gray")
...
# Filter genes
## n_cells_counts = number of cells with non-zero counts for a gene

par(mfrow=c(2,1),mar=c(5,4,1,1))
hist(rowData(sce)$n_cells_counts,breaks=50,freq=FALSE,col="gray")
hist(log10(1+rowData(sce)$n_cells_counts),breaks=50,freq=FALSE,col="gray")
par(mfrow=c(1,1))
...
fit = trendVar(sce,use.spikes=c(FALSE,NA)[2],loess.args=list(span=0.05))
