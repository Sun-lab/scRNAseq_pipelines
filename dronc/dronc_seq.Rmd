...
counts = data.table::fread(input = sprintf("zcat < %s",counts_fn))
...
Import clustering results. We utilize the supplemental files to label clusters. Refer to the file nmeth.4407-S10.xlsx.
...
k10_50_pcs = kmeans(reducedDim(sce_sub, "PCA"), 
					centers = 12, iter.max=1e3,nstart = 30,
					algorithm="MacQueen")
names(k10_50_pcs)
...
```{r cache=TRUE, warning = FALSE, message = FALSE, fig.height=14, fig.width=14}
library(SC3)
rowData(sce_sub)$feature_symbol = rowData(sce_sub)$external_gene_name
date()
all_ks = c(10,12)
# all_ks = 10
sce_sub = sc3(sce_sub, gene_filter=FALSE,
			n_cores = 1,kmeans_nstart = 50,
			kmeans_iter_max = 3e3,
